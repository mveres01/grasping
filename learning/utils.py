...
def load_dataset_hdf5(fname, n_train=-1, seed=1234):
    ...
    train_images = f['train']['images'][idx].astype(np.float32)
    train_grasps = f['train']['grasps'][idx].astype(np.float32)
    train_labels = f['train']['labels'][idx]
    train_props  = f['train']['object_props'][idx]

    test_images = f['test']['images'][:].astype(np.float32)
    test_grasps = f['test']['grasps'][:].astype(np.float32)
    test_labels = f['test']['labels'][:]
    test_props  = f['test']['object_props'][:]

    valid_images = f['valid']['images'][:].astype(np.float32)
    valid_grasps = f['valid']['grasps'][:].astype(np.float32)
    valid_labels = f['valid']['labels'][:]
    valid_props  = f['valid']['object_props'][:]
    f.close()

    train = [train_grasps, train_images, train_labels, train_props]
