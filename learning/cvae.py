...
def build_train_fcns(network, X1, Y, lrate, n_cll, beta1, beta2, **kwargs):
    ...
    #grads = T.grad(loss, net_params)
    #mgrads = lasagne.updates.total_norm_constraint(grads, max_norm=params['max_norm'])
    #cgrads = [T.clip(g, -params['clip_grad'], params['clip_grad']) for g in mgrads]
...
def train(network, X_train, X_test, X_val, params, train_fcns):
    ...
    for epoch in range(num_epochs):
        ...
        check_validation = epoch >= 30 and epoch%3 == 0
        valid_cll = 100000
        if check_validation:
        ...
        if epoch % 5 == 0:
            ...
            sample_grasps(valid_image[:5], valid_grasp[:5], valid_labels[:5], 
                          mu_sigma_fcn, epoch, params['save_dir'], n_samples=6)

            latent_outputs = []
            rz_mu, rz_ls, pz_mu, pz_ls, gz_mu, gz_ls = \
                stoch_vis_fcn(valid_image[:n], valid_grasp[:n])
